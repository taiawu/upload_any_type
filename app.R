                     ...
                     tags$style(HTML("hr {border-top: 1px solid #bdbdbd;}"))
                           ...
                           sidebarLayout( # Sidebar layout with input and output definitions
                               sidebarPanel(# Sidebar panel for inputs
                                   fileInput("file", p("Browse or drag-and-drop raw (RFU) DSF data", style = "font-family: 'Avenir Next'; font-size: 14px; color: black",align = "center"), 
                                             multiple = FALSE,
                                   ...
                                   selectInput("qT_channel", "Select channel (qTower only)",
                                               c("For qTower, select a channel" = "None",
                                                 ...
                                                 "SYPRO" = "SyPro")),
                                   tags$hr(),
                                       # p("Convert cycle number to temperature", style = "font-family: 'Avenir Next'; font-size: 14px; color: black", align = "center"),
                                        checkboxInput("cycle_to_T", "Convert cycle number to temperature", FALSE),
                                   splitLayout(
                                       numericInput(inputId="start_T", label="Starting Temp (ºC)", value = 25),
                                       numericInput(inputId="increment_T", label="Increase per cycle (ºC)", value = 1)
                                   ),
                               ...
                               mainPanel(
                                   ...
                                   tableOutput("opt_format") %>% withSpinner(color="#525252"), style = "overflow-x: scroll;overflow-y: scroll;height:580px"
                               ) # end main panel
                           )
) # end navbar page
